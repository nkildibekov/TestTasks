...
namespace Common.Services
{
    ...
    public class ClientResourceService : Control
    {
        ...
        public string GetResXFilePath(bool isLocal)
        {
            if(isLocal)
            {
            ...
            {
                return MapPathSecure(Path.Combine("~//bin/App_GlobalResources/",
                    GlobalResXFileName + ".resx"));
            }
            
        }

        public string GetJavaScript(bool isLocal)
        {
            if (!FileExists(isLocal))
            {
                return "alert(\"GlobalResourcesToJavaScript: Can't find the file " + GetResXFilePath(isLocal) + ".\");";
            }

            if (!string.IsNullOrEmpty(JavaScriptObjectName) && File.Exists(GetResXFilePath(isLocal)))
            {
                var script = new StringBuilder();
                using (System.Resources.ResXResourceReader resourceReader = new System.Resources.ResXResourceReader(GetResXFilePath(isLocal)))
                {
            ...
            return "alert('GlobalResourcesToJavaScript: Could not load the resource file " + GetResXFilePath(isLocal) + ".');";
        }
        #endregion

        #region Helper Methods
        public bool FileExists(bool isLocal)
        {
            if (!File.Exists(GetResXFilePath(isLocal)))
                return false;
            else
                return true;
        }
